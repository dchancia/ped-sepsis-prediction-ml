...
from utils.preprocess_utils import preprocess_der, preprocess_val
from utils.train_val_utils import external_val
import warnings
...
if __name__ == "__main__":
    ...
    for screening_method in screening_methods:
        ...
        metrics_true_df = pd. DataFrame(metrics_true)
        metrics_true_df.columns = ['Model', 'Accuracy', 'F1-Score', 'Recall', 'Precision', 'Specificity', 'AUROC', 'AUPRC', 'NPV', 'PPV']
        metrics_df.to_csv(os.path.join(path, screening_method, save_folder, 'metrics_external.csv'), index=False)
        metrics = compute_metrics (metrics_df, metrics_true_df, names)
        metrics.to_csv(os.path.join(path, screening_method, save_folder, 'sum_metrics_external.csv'), index=False)

        # Fix sensitivity at 85
        metrics, metrics_true = external_val (classifiers, X, y, data_val, names, True, colors, path, save_folder, screening_method, reps, col_names_fixed)
        ...
        metrics_true_df = pd. DataFrame(metrics_true)
        metrics_true_df.columns = ['Model', 'Accuracy', 'F1-Score', 'Recall', 'Precision', 'Specificity', 'AUROC', 'AUPRC', 'NPV', 'PPV']
        metrics_df.to_csv(os.path.join(path, screening_method, save_folder, 'metrics_external_fixed.csv'), index=False)
        metrics = compute_metrics (metrics_df, metrics_true_df, names)
        metrics.to_csv(os.path.join(path, screening_method, save_folder, 'sum_metrics_external_fixed.csv'), index=False)
