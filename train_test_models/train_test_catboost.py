...
from utils.evaluation_utils import eval_metrics
...
if __name__ == "__main__":
    ...
    eval_metrics(y_val, preds, probas)

    # Fix sensitivity at 85%
    new_thresh = find_thresh(y_val, probas, None, None)
    model.set_probability_threshold(new_thresh)
    model.save_model('./models/sepsis_catboost.cbm')

    # Evaluate on external validation
    model = CatBoostClassifier()
    model.load_model('./models/sepsis_catboost.cbm')
    preds = model.predict(X_val)
    eval_metrics(y_val, preds, probas)
